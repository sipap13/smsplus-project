...
export default function CdrMmg() {
  ...
  const fetchPage = useCallback(async (page, filters) => {
    setLoading(true);
    setError('');
    ...
    try {
      const res = await api.get(`/cdr/mmg?${params.toString()}`);
      setData(res.data.data || []);
      setTotal(res.data.total ?? 0);
      setCurrentPage(res.data.current_page ?? page);
      setLastPage(res.data.last_page ?? 0);
    } catch {
      setError("Impossible de charger les CDR MMG (droits ou API).");
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const clearFilters = () => {
    ...
    setEventStatus('');
    setSubscriberType('');
    setApplied({ start_date: '', event_status: '', subscriber_type: '' });
    setCurrentPage(1);
  };
  ...
  const handleExport = async () => {
    await downloadExcel(
      '/export/mmg',
      applied,
      `CDR_MMG_${new Date().toISOString().slice(0, 10)}.xlsx`,
      () => {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">Journaux CDR MMG</h1>
      ...
      <div className="command-bar" style={{ marginBottom: '1rem' }}>
        ...
        <div className="field">
          <div className="field-label">Statut événement</div>
          <select className="field-control" value={eventStatus} onChange={(e) => setEventStatus(e.target.value)}>
            <option value="">(tous)</option>
            {EVENT_STATUS_OPTIONS.map((k) => (
