$pumlPath = Join-Path $PSScriptRoot '..\docs\class-diagram.puml' | Resolve-Path
$pngPath = Join-Path $PSScriptRoot '..\docs\class-diagram.png'
$svgPath = Join-Path $PSScriptRoot '..\docs\class-diagram.svg'

$text = Get-Content -Path $pumlPath -Raw -Encoding UTF8
$bytes = [System.Text.Encoding]::UTF8.GetBytes($text)

# Compress using Deflate (raw) into memory
